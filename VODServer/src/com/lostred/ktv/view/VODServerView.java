...
import com.lostred.ktv.controller.VODSAction;
import com.lostred.ktv.controller.VODSWindow;
 ...
 * 点播主界面
 */
public class VODServerView extends JFrame {
    /**
     * 点播主界面单例对象
     */
    private static VODServerView VOD_SERVER_VIEW;
    /**
     * 开启服务端按钮
     */
     ...
     * 关闭服务端按钮
     */
    private final JLabel lbServer = new JLabel("播放服务端：未连接", JLabel.RIGHT);
    /**
     * 播放服务端消息文本域
     */
    private final JTextArea taPlayerServer = new JTextArea();
    /**
     * 点播客户端消息文本域
     */
    private final JTextArea taVODClient = new JTextArea();

    /**
     * 构造点播主界面
     */
    private VODServerView() {
        //初始化控件
        ...
        JLabel lbPlayerServer = new JLabel("播放服务端消息：");
        JLabel lbVODClient = new JLabel("点播客户端消息：");
        ...
        taPlayerServer.setEditable(false);
        taVODClient.setEditable(false);
        playerServerScrollPane.setViewportView(taPlayerServer);
        VODClientScrollPane.setViewportView(taVODClient);
        //添加控件
        ...
        //添加监听
        VODSWindow vodsWindow = new VODSWindow();
        VODSAction vodsAction = new VODSAction();
        addWindowListener(vodsWindow);
        btnStart.addActionListener(vodsAction);
        btnClose.addActionListener(vodsAction);
     ...
     * @return 点播主界面单例对象
     */
    public static VODServerView getVODServerView() {
        if (VOD_SERVER_VIEW == null) {
            synchronized (VODServerView.class) {
                if (VOD_SERVER_VIEW == null) {
                    VOD_SERVER_VIEW = new VODServerView();
                }
            }
        }
        return VOD_SERVER_VIEW;
    }
    /**
     * 改变服务器状态
     *
    ...
    public JTextArea getTaPlayerServer() {
        return taPlayerServer;
    }

    public JTextArea getTaVODClient() {
        return taVODClient;
    }
}
